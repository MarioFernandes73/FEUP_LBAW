...
function addAImagesComment($idfile, $idcomment)
{
    global $conn;
    $stmt = $conn->prepare("INSERT INTO \"FileComment\" (idcomment,idfile) VALUES (?,?)");
    $stmt->execute(array($idcomment, $idfile));
}

function addFileToTicketComment($idfile, $idticketcomment)
{
    global $conn;
    $stmt = $conn->prepare("INSERT INTO \"FileTicketComment\" (idfile,idticketcomment) VALUES (?,?)");
